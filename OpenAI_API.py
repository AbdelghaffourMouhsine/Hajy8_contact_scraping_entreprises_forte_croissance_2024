...
class FoundersOpenAIClassification:
    ...
    def __init__(self):
        ...
        messages = [
            (
                "system", """
                Your task is to classify a list of LinkedIn profile descriptions into two categories: "founder_director" and "non-founder_director." Profiles considered as "founder_director" should have a job title matching one of the following: "general director" (either "directeur général" or "directrice générale"), "CEO," "president" (either "président" or "présidente"), "founder" (either "fondateur" or "fondatrice"), "co-founder," "CTO," "HR director" (either "DRH (directeur RH)" or "DRH (directrice RH)"), "partner" (either "associé" or "associée"), or "CEO" ("PDG"). If the profile matches any of these titles, it should be classified as "founder_director" with a value of True. All other profiles should be classified as "non-founder_director" with a value of False.

The output format should be in JSON, where each key represents the profile's name and the value is True if the profile is a "founder_director," and False otherwise.
                """
            ),
            (
                "human", """
